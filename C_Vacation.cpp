...
int solve()
{
	...
	for(int i=2;i<=n;i++)
	{
		...
		dp[i][0]=max(dp[i][0], max(dp[i-1][1], dp[i-1][2])+a);
		dp[i][1]=max(dp[i-1][1], dp[i-2][1]+b);
		dp[i][1]=max(dp[i][1], max(dp[i-1][0], dp[i-1][2])+b);
		dp[i][2]=max(dp[i-1][2], dp[i-2][2]+c);
		dp[i][2]=max(dp[i][2], max(dp[i-1][0], dp[i-1][1])+c);
	}
